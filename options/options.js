...
// Toggle API section visibility based on free mode
...
// Load settings when page opened
...
document.querySelectorAll(".toggle-password").forEach((button) => {
  button.addEventListener("click", function () {
    ...
    if (input.type === "password") {
      ...
      input.type = "password";
      this.innerHTML =
        '<svg class="eye-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>';
    }
  });
});
...
function saveSettings(e) {
  ...
  const settings = {
    ...
    translationColor:
      document.querySelector('input[name="translationColor"]:checked')?.value ||
      "default",
...
document
  .getElementById("testGoogleBtn")
  .addEventListener("click", () => testAPI("google"));
document
  .getElementById("testAzureBtn")
  .addEventListener("click", () => testAPI("azure"));
