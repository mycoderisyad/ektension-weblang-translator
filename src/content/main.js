...
if (!__g.__WEBLANG_CS_INIT) {
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.type === 'SHOW_AI_POPUP') {
      try {
        // Inject AI popup CSS if not already injected
        if (!document.querySelector('#weblang-ai-popup-styles')) {
          injectAIPopupCSS();
        }
        
        // Create and show AI popup
    ...
    if (msg.type === 'RESTORE_ORIGINAL') {
      try {
        ...
        // Reset translation pipeline state after restore
        TranslationPipeline.clearCache();
  ...
  function restoreOriginalText() {
    ...
    // Remove translation elements
    translations.forEach(el => {
      el.remove();
      count++;
    });
    
    // Restore original text for replaced elements
    ...
    // Clear any remaining weblang elements
    document.querySelectorAll('[class*="weblang"], [id*="weblang"]').forEach(el => {
      if (el.classList.contains('weblang-translation') || 
          el.id.includes('weblang') || 
          el.className.includes('weblang')) {
        el.remove();
      }
    });
...
// Function to inject AI popup CSS
