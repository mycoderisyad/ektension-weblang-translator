// Minimal popup runtime to control translation, settings sync, and quick-translate

function sendToContent(msg, timeout = 5000) {
...
async function handleAIRequest(aiType, statusMessage) {
  try {
    ...
    // Get translated content from page
    ...
    if (response && response.success) {
      // Show result in AI popup instead of alert
...
async function loadPrefsIntoUI() {
  return new Promise((resolve) => {
    chrome.storage.sync.get(['sourceLang','targetLang','translationMode','quickTranslateEnabled','translationColor','provider','useFreeMode','googleKey','azureKey'], (d) => {
      try {
        const s = document.getElementById('sourceLang'); if (s) s.value = d.sourceLang || 'auto';
        const t = document.getElementById('targetLang'); if (t) t.value = d.targetLang || 'id';
        const m = document.getElementById('mode'); if (m) m.value = d.translationMode || 'paragraph';
        const sw = document.getElementById('qtSwitch'); if (sw) sw.checked = d.quickTranslateEnabled !== false;
...
async function ensureContentScript() {
  try {
    ...
    const testResponse = await sendToContent({ type: 'PING' }, 2000);
    ...
    if (tabs && tabs[0]) {
      ...
      const testAgain = await sendToContent({ type: 'PING' }, 2000);
...
async function init() {
  ...
  // Poll translation status every 500ms
  ...
  if (btn) btn.addEventListener('click', async () => {
    try {
      ...
      if (status && status.isTranslating) {
        // Currently translating, so stop it
      ...
      // Reset button after translation completes
      updateTranslateButton(false);
...
// Minimal popup bundle (logic already exists in root popup file if present)
export {};
